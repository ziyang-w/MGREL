...
if 'HGT_result' not in os.listdir(path='..'):
    os.mkdir('../HGT_result')
...
for SEED in range(10):
    ...
    for (train_idx, val_idx) in kf.split(data.cpu().numpy(), label.cpu().numpy()):
        ...
        model = HGT(in_feats=[feature['gene'].shape[1],
    ...
    #              columns=['Predict', 'Label']).to_csv('HAN_predictions_{}.csv'.format(SEED),
    ...
    for ratio in [1, 3, 5, 10, 20]:
        pd.DataFrame(np.array([pred_result[ratio], pred_label[ratio]]).T,
                     columns=['Predict', 'Label']).to_csv('../HGT_result/HGT_predictions_ratio{}_{}.csv'.format(ratio,
                                                                                                                SEED),
                                                          index=False)
