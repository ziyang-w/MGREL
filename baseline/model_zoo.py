...
class HGT(nn.Module):
    def __init__(self, in_feats, node_dict, edge_dict):
        ...
        self.gene_emb = nn.Linear(in_feats[0], 32, bias=False)
        self.dis_emb = nn.Linear(in_feats[1], 32, bias=False)
        self.embedding = HGTLayer(32, 32, node_dict, edge_dict, 2, 0.1)
        self.relu = nn.ReLU()
        self.weights = nn.Linear(32, 32, bias=False)
        nn.init.xavier_uniform_(self.weights.weight)
...
class HAN(nn.Module):
    def __init__(self, meta_paths, in_feats):
        ...
        self.gene_emb = nn.Linear(in_feats[0], 64, bias=False)
        self.dis_emb = nn.Linear(in_feats[1], 64, bias=False)
        self.embedding_1_gene = HANLayer([meta_paths[0]], 64, 32, 5, 0.1)
        self.embedding_1_dis = HANLayer([meta_paths[1]], 64, 32, 5, 0.1)
        self.gene_emb_1 = nn.Linear(32 * 5, 32, bias=False)
        self.dis_emb_1 = nn.Linear(32 * 5, 32, bias=False)
        self.embedding_2_gene = HANLayer([meta_paths[0]], 32, 32, 5, 0.1)
        self.embedding_2_dis = HANLayer([meta_paths[1]], 32, 32, 5, 0.1)
        self.gene_emb_2 = nn.Linear(32 * 5, 32, bias=False)
    ...
    def forward(self, g, feature):
        ...
        h['gene'] = self.gene_emb_1(self.embedding_1_gene(g, h['gene']))
        h['disease'] = self.dis_emb_1(self.embedding_1_dis(g, h['disease']))
        h['gene'] = self.gene_emb_2(self.embedding_2_gene(g, h['gene']))
